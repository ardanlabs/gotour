...
// μεταβλητού μήκους VLQ και τον αποκωδικοποιεί σε έναν ακέραιο
// με 32 bit.
func DecodeVarint(input []byte) (uint32, error) {
	...
	for i := len(input) - 1; i >= 0; i-- {
		...
		// Επεξεργαστείτε τα πρώτα 7 bit και αγνοήστε το 8ο.
		for checkBit := 0; checkBit < 7; checkBit++ {

			// Απομακρύνετε περιστρέφοντας το τελευταίο bit και μετακινείστε
			// το στο τέλος.
			// Πριν: 0000 0001
			// Μετά: 1000 0000
			n = bits.RotateLeft8(n, -1)

			// Υπολογίστε με βάση μόνο εκείνα τα bit τιμής 1, που περιστράφηκαν.
			// Μετατρέψτε την bitPos σε αρίθμηση με βάση το 10.
			if n >= lastBitSet {
				switch {
				case bitPos == 0:
					d++
...
func EncodeVarint(n uint32) []byte {
	...
	for bytePos := maxBytes - 1; bytePos >= 0; bytePos-- {
		...
		for checkBit := 0; checkBit < 7; checkBit++ {
			...
			// Υπολογίστε μόνο με βάση εκείνων των bit τιμής 1,
