...
// Αλλάξτε τον κώδικα από το πρότυπο. Προσθέστε ένα νέο τύπο CachingFeed που
// ενσωματώνει τον Feed και παρακάμπτει την μέθοδο τύπου Fetch.

// Αυτό το πρόγραμμα ορίζει έναν τύπο Feed με δύο μεθόδους: την Count και
// την Fetch. Δημιουργείστε έναν νέο τύπο CachingFeed που ενσωματώνει τον
// *Feed αλλά ξαναορίζει (στμ. override) την μέθοδο τύπου Fetch.
//
// Ο τύπος CachingFeed θα πρέπει να έχει να έχει έναν σχεσιακό πίνακα με
// Documents, προκειμένου να περιοριστεί το πλήθος των κλήσεων στην Feed.Fetch.
package main
...
// Ο FetchCounter είναι η συμπεριφορά στην οποία εξαρτόμαστε για την
// συνάρτηση επεξεργασίας μας.
...
func NewCachingFeed(f *Feed) *CachingFeed {
	...
	// TODO δημιουργείστε έναν CachingFeed με έναν σχεσιακό πίνακα με αρχική τιμή και
	// ενσωματωμένο feed.
...
// Η Fetch καλεί την μέθοδο τύπου Fetch του ενσωματωμένου τύπου, αν το κλειδί δεν είναι
// στην προσωρινή μνήμη.
func (cf *CachingFeed) Fetch(key string) (Document, error) {

	// TODO υλοποιείστε αυτή την μέθοδο τύπου. Ελέγξτε το πεδίο του σχεσιακού
	// πίνακα για το συγκεκριμένο κλειδί και επιστρέψτε το αν βρεθεί.
	// Αν δεν βρεθεί, καλέστε την μέθοδο τύπου Fetch του ενσωματωμένου τύπου.
	// Αποθηκεύστε το αποτέλεσμα στον σχεσιακό πίνακα, πριν το επιστρέψετε.
